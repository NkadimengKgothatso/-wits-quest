...
3. **Location State Rules**:
   ...
   - If distance `d` is greater than 25 meters: set status to `OUT_OF_RANGE` and display remaining distance (for example, _"Distance: 85 meters away. Walk closer to unlock"_).
...
2. **Card Creation Interface**:
   ...
   - Card Name (for example, _Cullen Library Rare Archives_).
   - Category (_Science_, _History_, _Landmarks_, _Lifestyle_, _Sports_).
   - Rarity (_Common_, _Rare_, _Epic_, _Legendary_).
