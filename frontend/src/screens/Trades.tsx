...
const RARITY_BORDER: Record<string, string> = {
  Legendary: '#fed6ce', Epic: '#a78bfa', Rare: '#60a5fa', Common: 'rgba(164,181,209,0.4)',
};
...
function CardSlot({
  ...
  return (
    <div style={{
      flex: 1,
      borderRadius: 14,
      border: `1.5px ${card ? 'solid' : 'dashed'} ${card ? RARITY_BORDER[card.rarity] : 'rgba(164,181,209,0.3)'}`,
      background: card ? 'rgba(73,104,148,0.3)' : 'rgba(29,49,86,0.3)',
      padding: 14,
      textAlign: 'center',
      minHeight: 160,
      display: 'flex',
      ...
      {card ? (
        <>
          <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'rgba(29,49,86,0.8)', border: `2px solid ${RARITY_BORDER[card.rarity]}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: RARITY_BORDER[card.rarity], fontWeight: 800, fontSize: 14 }}>
          ...
          <div style={{ fontSize: 11, fontWeight: 800, color: '#fed6ce', marginTop: 2 }}>
          ...
          {onClear && (
            <button
              ...
              style={{
                ...
                background: 'rgba(73,104,148,0.5)', border: 'none',
        ...
        <div style={{ color: 'rgba(164,181,209,0.4)', fontSize: 12, fontWeight: 600 }}>
          Select a Card
        </div>
      )}
    </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '14px 16px 0', maxWidth: 600, margin: '0 auto' }}>
        ...
        <p style={{ fontSize: 12, color: '#a4b5d1', margin: '0 0 16px' }}>P2P card exchange with stat fairness calculation</p>
      </div>
      ...
      <div style={{ padding: '0 16px', maxWidth: 600, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div className="glass-dark" style={{ borderRadius: 16, padding: 16 }}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 16 }}>
            <CardSlot card={myOffer} label="YOU OFFER" onClear={() => setMyOffer(null)} accent="#fed6ce" />
            <div style={{ fontSize: 14, fontWeight: 900, color: '#b0cbe6' }}>↔</div>
            <CardSlot card={theirOffer} label="THEIR OFFER" onClear={() => setTheirOffer(null)} accent="#b0cbe6" />
          </div>
          ...
          {myOffer && theirOffer && (
            <div style={{
              ...
              background: fair ? 'rgba(176,203,230,0.15)' : 'rgba(254, 214, 206, 0.15)',
              border: `1px solid ${fair ? 'rgba(176,203,230,0.4)' : 'rgba(254, 214, 206, 0.4)'}`,
            }}>
              <div style={{ fontSize: 12, fontWeight: 800, color: fair ? '#b0cbe6' : '#fed6ce' }}>
          ...
          {tradeDone ? (
            <div style={{ textAlign: 'center', color: '#fed6ce', fontWeight: 800, padding: 10 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <div className="glass-dark" style={{ borderRadius: 14, padding: 12 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: '#fed6ce', marginBottom: 8, textTransform: 'uppercase' }}>
              ...
              {MY_CARDS.map((c) => (
                <button
                  ...
                  style={{
                    ...
                    background: myOffer?.id === c.id ? 'rgba(254,214,206,0.2)' : 'rgba(73,104,148,0.25)',
                    border: `1px solid ${myOffer?.id === c.id ? '#fed6ce' : 'rgba(164,181,209,0.15)'}`,
                    color: 'white', textAlign: 'left', cursor: 'pointer',
                  }}
                >
                  <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#1d3156', border: `1px solid ${RARITY_BORDER[c.rarity]}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: RARITY_BORDER[c.rarity], fontWeight: 800, fontSize: 9 }}>
                    {c.name.substring(0, 2).toUpperCase()}
                  </div>
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 700 }}>{c.name}</div>
                    <div style={{ fontSize: 9, color: '#a4b5d1' }}>×{c.owned} · {statTotal(c)} pts</div>
                  </div>
                </button>
              ))}
            </div>
          ...
          {/* Partner cards */}
          <div className="glass-dark" style={{ borderRadius: 14, padding: 12 }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: '#b0cbe6', marginBottom: 8, textTransform: 'uppercase' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {THEIR_CARDS.map((c) => (
                <button
                  ...
                  style={{
                    display: 'flex', alignItems: 'center', gap: 8, padding: 8, borderRadius: 8,
                    background: theirOffer?.id === c.id ? 'rgba(176,203,230,0.2)' : 'rgba(73,104,148,0.25)',
                    border: `1px solid ${theirOffer?.id === c.id ? '#b0cbe6' : 'rgba(164,181,209,0.15)'}`,
                    color: 'white', textAlign: 'left', cursor: 'pointer',
                  }}
                >
                  <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#1d3156', border: `1px solid ${RARITY_BORDER[c.rarity]}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: RARITY_BORDER[c.rarity], fontWeight: 800, fontSize: 9 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 700 }}>{c.name}</div>
                    <div style={{ fontSize: 9, color: '#a4b5d1' }}>{statTotal(c)} pts</div>
                  </div>
                </button>
              ))}
            </div>
