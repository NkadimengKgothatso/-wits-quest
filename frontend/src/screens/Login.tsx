...
const LANDMARKS = [
  { id: 'uncorner', emoji: '🏬', label: 'University Corner', x: 12, y: 52, delay: 0 },
  { id: 'cullen', emoji: '📚', label: 'W. Cullen Library', x: 30, y: 28, delay: 0.55 },
  { id: 'wartenweiler', emoji: '📖', label: 'Wartenweiler Library', x: 40, y: 20, delay: 1.1 },
  { id: 'planetarium', emoji: '🪐', label: 'Planetarium', x: 66, y: 22, delay: 1.65 },
  { id: 'greathall', emoji: '🏛️', label: 'Great Hall', x: 48, y: 48, delay: 2.2 },
  { id: 'smhouse', emoji: '🏢', label: 'Solomon Mahlangu House', x: 56, y: 56, delay: 2.75 },
  { id: 'sturrock', emoji: '🌳', label: 'Sturrock Park', x: 78, y: 44, delay: 3.3 },
  { id: 'clm', emoji: '🎓', label: 'Law & Management', x: 62, y: 78, delay: 3.85 },
  { id: 'origins', emoji: '🏺', label: 'Origins Centre', x: 24, y: 76, delay: 4.4 },
];
      ...
      {!showAuth ? (
        <div className="cinematic-stage" style={{ position: 'relative', zIndex: 10, width: '100%', maxWidth: 760, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <div
            ...
            style={{
              width: '100%', aspectRatio: '4 / 3.1', maxHeight: '62vh',
          ...
          >
            ...
            {/* Map container */}
            <div className={seqStarted ? 'map-reveal' : 'map-hidden'} style={{ position: 'absolute', inset: 18, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <svg viewBox="0 0 100 90" style={{ width: '100%', height: '100%' }}>
                {/* Title banner */}
                <text x="50" y="8" textAnchor="middle" fontSize="5" fill="#6b5630" fontWeight="800" letterSpacing="0.5">WITS EAST CAMPUS QUEST MAP</text>

                {/* Grounds / lawns */}
                <rect x="4" y="12" width="92" height="74" rx="6" fill="rgba(107, 125, 44, 0.12)" />

                {/* Ring road loop */}
                <path
                  d="M 10 45 Q 50 10 90 45 Q 50 80 10 45"
                  fill="none" stroke="rgba(168, 125, 77, 0.22)" strokeWidth="1.5" strokeDasharray="2 2"
                />

                {/* Jan Smuts Avenue boundary */}
                <line x1="4" y1="12" x2="4" y2="86" stroke="rgba(107, 86, 48, 0.25)" strokeWidth="1.5" />
                <text x="3" y="50" textAnchor="end" fontSize="3" fill="#a87d4d" transform="rotate(-90 3 50)">Jan Smuts Ave</text>

                {/* Zone blocks approximating campus buildings */}
                <rect x="26" y="20" width="16" height="10" rx="1.5" fill="rgba(168, 125, 77, 0.22)" />
                <rect x="36" y="16" width="12" height="10" rx="1.5" fill="rgba(168, 125, 77, 0.22)" />
                <rect x="62" y="18" width="14" height="12" rx="1.5" fill="rgba(168, 125, 77, 0.22)" />
                <rect x="44" y="40" width="16" height="14" rx="1.5" fill="rgba(220, 166, 104, 0.25)" />
                <rect x="52" y="52" width="14" height="12" rx="1.5" fill="rgba(168, 125, 77, 0.22)" />
                <rect x="72" y="38" width="12" height="14" rx="1.5" fill="rgba(107, 125, 44, 0.22)" />
                <rect x="58" y="72" width="16" height="10" rx="1.5" fill="rgba(168, 125, 77, 0.22)" />
                <rect x="20" y="72" width="14" height="10" rx="1.5" fill="rgba(168, 125, 77, 0.22)" />

                {/* Central plaza / great lawn */}
                <ellipse cx="50" cy="50" rx="8" ry="6" fill="rgba(107, 125, 44, 0.16)" />
                ...
                {LANDMARKS.map((lm) => (
                  <g key={lm.id} className={seqStarted ? 'landmark-pop' : 'landmark-hidden'} style={{ animationDelay: `${4.8 + lm.delay}s` }}>
                    <circle cx={lm.x} cy={lm.y} r="5.5" fill="#f5e6cc" stroke="#dca668" strokeWidth="0.8" />
                    <text x={lm.x} y={lm.y + 1.4} textAnchor="middle" fontSize="5.5">{lm.emoji}</text>
                    <text x={lm.x} y={lm.y + 11} textAnchor="middle" fontSize="3.2" fill="#6b5630" fontWeight="700">{lm.label}</text>
                  </g>
                ))}
            ...
            <div
              ...
              style={{ position: 'absolute', left: '78%', top: '74%', width: 64, height: 64, transform: 'translate(-50%, -50%)' }}
            >
          ...
          {/* Tagline */}
          <div style={{ textAlign: 'center', marginTop: 28, maxWidth: 620 }}>
            <h1
              className={seqStarted ? 'text-reveal' : 'text-hidden'}
              style={{
                fontSize: 'clamp(24px, 6vw, 42px)', fontWeight: 900, color: '#f8f2e8',
            ...
            <p
              ...
              style={{
                fontSize: 'clamp(14px, 3vw, 18px)', color: '#dca668', margin: 0,
          ...
          <div className={seqStarted ? 'text-reveal-delay-2' : 'text-hidden'} style={{ display: 'flex', gap: 12, marginTop: 24 }}>
            <button
              onClick={() => { setAuthMode('login'); setShowAuth(true); }}
              className="btn-peach"
